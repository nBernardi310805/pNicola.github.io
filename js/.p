...
addEventListener('DOMContentLoaded', darkmode)
function darkmode(){
    if(localStorage.getItem('ultraDM')=='true')
        document.querySelector('style').innerHTML = 'body{background-color:#000000;}'
}
function cmd(){
    nam=' '
    logg=' '
    ...
    switch(com){
        ...
        case 'I am a hero':
            ...
            break
        case 'pwd':
            alert('github.com/nBernardi310805/pNicola.github.io/index.html')
            break
        case 'godMode=true':
            ...
            break
        case 'bingotop':
            alert('è una password?!?')
            break
        case 'template':
            alert('Usa il mio template gratis per il tuo sito, incolla la seguente riga di codice:')
            alert('<link rel="stylesheet" href="https://portalenicola.it/css/template.css">')
            break
        case 'Gianchio98', 'gianchio98', 'gianchio', 'Gianchio':
            alert('Forse intendevi "Granchio98" :)')
            break
        case 'Granchio', 'granchio':
            alert('98?')
            break
        case 'oleddarkmode':
            localStorage.setItem('ultraDM', true)
            alert('Ultra darkmode (beta) attivata. Disattivala con "oleddarkmode --off"')
            darkmode()
            break
        case 'oleddarkmode --off':
            localStorage.clear('utraDM')
            alert('Ultra darkmode disattivata')
            darkmode()
            break
    }
}
